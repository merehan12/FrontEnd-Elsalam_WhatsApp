...
//   "https://chat-backend-7isxn.ondigitalocean.app/api"; // <= backend /api
...
//   (process.env.REACT_APP_WS_BASE && process.env.REACT_APP_WS_BASE.replace(/\/+$/, "")) ||
...
//   get(key) {
//     try {
//       if (_session && _session.getItem(key) != null) return _session.getItem(key);
...
//   if (status === 403) {
//     const code = String(error?.response?.data?.code || error?.response?.data?.detail || "").toLowerCase();
//     return code.includes("token");
//   }

//   if (status === 400) {
//     const msg = String(error?.response?.data?.detail || "").toLowerCase();
